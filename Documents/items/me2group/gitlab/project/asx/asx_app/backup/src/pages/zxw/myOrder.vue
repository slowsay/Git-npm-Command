...
export default {
          ...
          boxData: [
            {
              ...
              shopName:
                "专柜洋风二代陶瓷贺卡九回复客户卡合法空间哈开始的很疯狂哈萨克发ashkdfhkjah",
              color: "红色",
              area: "5.5",
              style: "配餐具",
              money: "1888.88",
  ...
  methods: {
    ...
    onLoad(index) {
      ...
      setTimeout(() => {
        for (let i = 0; i < 10; i++) {
          this.formData[index].boxData.push(
            this.formData[index].boxData.length + 1
          );
        }
        // 加载状态结束
        this.loading = false;
        // 数据全部加载完成
        if (this.formData[index].boxData.length >= 13) {
          this.finished = true;
        }
      }, 500);
    },
    //点击跳转到详情页
    gotoDetail(index,index1) {
