...
<template>
  <div class="picker-area" @blur="hideList" tabindex="1" id="picker">
    <div class="picker-show">
      <span
        :class="['picker-province', chooseIndex == 0?'pressActive':'']"
      ...
      <span
        :class="['picker-city', chooseIndex  == 1?'pressActive':'']"
        :style="{'max-width': maxLen}"
        @click="selectClick(1)"
        v-if="showLevel(2)"
        :title="city"
      ...
      <span
        :class="['picker-county', chooseIndex == 2?'pressActive':'']"
        @click="selectClick(2)"
        v-if="showLevel(3)"
        :title="county"
        :style="{'max-width': maxLen}"
      ...
      <span
        :class="['picker-road', chooseIndex == 3?'pressActive':'']"
        @click="selectClick(3)"
        v-if="showLevel(4)"
        :title="road"
        :style="{'max-width': maxLen}"
    ...
    <ul class="picker-list" ref="picker-list" v-if="showList && showDataArray.length > 0">
      <li
        v-for="item in showDataArray"
        :key="item.code"
        @click="chooseOption(item.code,item.name)"
      >{{item.name}}</li>
    </ul>
  </div>
</template>
